  ...
  _createClass(BootstrapCommand, [{
    ...
    value: function execute(callback) {
      ...
      this.bootstrapPackages(function (err) {
        if (err) {
          ...
          _this2.logger.success("Successfully bootstrapped " + _this2.packagesToBootstrap.length + " packages.");
          callback(null, true);
        }
      });
    }
    ...
    value: function bootstrapPackages(callback) {
      ...
      if (this.flags.includeFilteredDependencies) {
        this.packagesToBootstrap = _PackageUtilities2.default.addDependencies(this.filteredPackages, this.packageGraph);
      }

      this.logger.info("Bootstrapping " + this.packagesToBootstrap.length + " packages");
      this.batchedPackages = this.toposort ? _PackageUtilities2.default.topologicallyBatchPackages(this.packagesToBootstrap, this.logger) : [this.packagesToBootstrap];
    ...
    value: function getDependenciesToInstall() {
      ...
      var findPackage = function findPackage(name, version) {
        return (0, _lodash2.default)(_this5.packages, function (pkg) {
      ...
      var scope = _getOptions.hoist;
      var ignore = _getOptions.nohoist;
      ...
      Object.keys(depsToInstall).forEach(function (name) {
        ...
        var versions = _depsToInstall$name.versions;
        var dependents = _depsToInstall$name.dependents;
    ...
    value: function installExternalDependencies(callback) {
      ...
      var _getDependenciesToIns = this.getDependenciesToInstall(this.packagesToBootstrap);

      var leaves = _getDependenciesToIns.leaves;
      var root = _getDependenciesToIns.root;
      ...
      if (Object.keys(root).length) {
        ...
        actions.push(function (cb) {
          _async2.default.series(root.map(function (_ref7) {
            var name = _ref7.name;
            var dependents = _ref7.dependents;
            return function (cb) {
      ...
      Object.keys(leaves).map(function (pkgName) {
        ...
        return actions.push(function (cb) {
          return _NpmUtilities2.default.installInDir(pkg.location, deps, function (err) {
    ...
    value: function symlinkPackages(callback) {
      ...
      this.progressBar.init(this.packagesToBootstrap.length);
      var actions = [];
      this.packagesToBootstrap.forEach(function (filteredPackage) {
